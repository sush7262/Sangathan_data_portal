...
function naReview() {
  ...
  const secRows = SECTIONS.map(s => {
    const p = sectionPct(d, s); const st = p === 100 ? 'Complete' : (p > 0 ? 'Draft' : 'Pending');
    ...
    return `<tr><td>${s.short} <span class="muted">· ${esc(s.title.split('—')[1] || '')}</span></td>
      ...
      <td class="right num">${p}%</td><td class="right"><span class="pill ${cl}">${st}</span></td></tr>`;
  }).join('');
  $('#app').innerHTML = topbar() + `<div class="content" style="margin:0 auto">
    ${stepper('review')}
...
function adReports() {
  ...
  const preview = DNAMES.map(d => {
    ...
    return `<tr><td class="dev">${d}</td>${cells}<td class="right"><span class="pill ${STCLASS[st]}">${STLABEL[st]}</span></td></tr>`;
  }).join('');
...
function exportCSV() {
  ...
  DISTRICTS.forEach(d => {
    const e = ENTRIES[d.name]; if (!e) return;
    SECTIONS.forEach(s => s.cols.forEach(c => {
      const v = e.values[c]; if (v == null || v === '') return;
